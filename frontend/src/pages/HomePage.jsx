  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-3">
          <div className="relative group hidden sm:block">
            ...
            <input 
              ...
              className="pl-10 pr-4 py-2 w-64 bg-brand-elevated border border-brand-border rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-brand-purple/50 focus:border-brand-purple/50 transition-all text-brand-text placeholder-brand-muted"
            />
          </div>
          <button className="p-2.5 bg-brand-elevated border border-brand-border rounded-xl hover:bg-brand-elevated transition-colors relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-brand-surface border border-brand-border rounded-2xl p-6 relative overflow-hidden group">
          ...
          <h2 className="text-sm font-semibold text-brand-muted mb-6 flex items-center">
            <Briefcase className="w-4 h-4 mr-2" /> PORTFOLIO SUMMARY
          </h2>
          
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div>
              <p className="text-xs text-brand-muted opacity-80 mb-1">Total Balance</p>
            ...
            <div>
              ...
              <p className={`text-lg font-semibold ${!analytics?.stats ? 'text-brand-muted' : analytics.stats.totalPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                ${analytics?.stats ? analytics.stats.totalPnL.toFixed(2) : '0.00'}
              </p>
            </div>
            <div>
              <p className="text-xs text-brand-muted opacity-80 mb-1">Today's P/L</p>
              <p className={`text-lg font-semibold ${!analytics?.stats ? 'text-brand-muted' : analytics.stats.todayPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className="mt-8 pt-6 border-t border-brand-border">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-medium text-brand-text">Performance (30d)</h3>
            </div>
            <div ref={chartContainerRef} className="w-full h-[300px] relative">
               {(!analytics?.equityCurve || analytics.equityCurve.length === 0) && (
                 <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                   <p className="text-sm text-zinc-600 bg-brand-surface/80 px-4 py-2 rounded-lg backdrop-blur-sm border border-brand-border">No trade data available</p>
                 </div>
               )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          {/* AI Insight Card */}
          <div className="bg-gradient-to-br from-brand-elevated to-brand-surface border border-brand-purple/20 rounded-2xl p-6 relative overflow-hidden">
            ...
            {insightLoading ? (
              <div className="animate-pulse space-y-2">
                <div className="h-4 bg-brand-elevated rounded w-3/4"></div>
                <div className="h-4 bg-brand-elevated rounded w-full"></div>
                <div className="h-4 bg-brand-elevated rounded w-5/6"></div>
              </div>
            ) : (
          ...
          <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-brand-muted flex items-center">
        ...
        <div className="lg:col-span-3 bg-brand-surface border border-brand-border rounded-2xl p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="text-brand-muted opacity-80 border-b border-brand-border">
              ...
              <tbody>
                {openTrades.length === 0 ? (
                  <tr>
                    <td colSpan="5" className="py-8 text-center text-brand-muted opacity-80">
                  ...
                  openTrades.map(trade => {
                    ...
                    return (
                      <tr key={trade._id} className="text-brand-text hover:bg-brand-elevated/50 transition-colors border-b border-brand-border/50 last:border-0">
                        ...
                        <td className="py-3 font-mono text-brand-muted">{currentPrice || '--'}</td>
                        <td className="py-3 text-right">
                          <div className="flex items-center justify-end space-x-3">
                            <span className={`font-bold block ${livePnl === null ? 'text-brand-muted opacity-80' : livePnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                              {livePnl === null ? 'Wait...' : `$${livePnl.toFixed(2)}`}
                            </span>
                            <button onClick={() => { setCurrentTrade(trade); setIsCloseModalOpen(true); }} className="p-1.5 text-brand-muted hover:text-green-400 bg-brand-elevated rounded-lg transition-colors" title="Close Trade">
